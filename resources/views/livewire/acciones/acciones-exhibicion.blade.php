    ...
    <x-dialog-modal wire:model='display'>
        ...
        <x-slot name='content'>
            <div class="row text-center">
                <div class="col-6">
                    <a href="{{ route('exhibicion.edit', $exhibicion) }}">
                        <button class="btn btn-primary">Editar exhibicion</button>
                    </a>
                </div>

                <div class="col-6">
                    <form action="{{ route('exhibicion.destroy', $exhibicion)}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">Eliminar exhibicion</button>
                    </form>
                </div>
            </div>
